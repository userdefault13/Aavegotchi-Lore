<template>
  <aside :class="$style.panel">
    ...
    <template v-else-if="status?.isBranch || status?.isFork">
      <router-link :to="`/tome/${status.upstreamChronicleId}`" :class="$style.upstreamLink">
      ...
      <GotchiTooltip v-if="loreBranchId" :tip="SYNC_PANEL.pushReview" block>
        <router-link
          :to="`/lore/${loreBranchId}/proposals?new=1`"
          class="btn-pixel text-[8px] w-full mt-2 block text-center"
        >
          Push for review
        </router-link>
      </GotchiTooltip>
      <GotchiTooltip v-if="loreBranchId" :tip="SYNC_PANEL.viewPrs" block>
        <router-link
          :to="`/lore/${loreBranchId}/proposals`"
          class="btn-pixel text-[8px] w-full mt-1 block text-center"
        >
          View PRs
        </router-link>
      </GotchiTooltip>
...
defineProps({
  ...
  chronicleId: { type: String, default: '' },
